...
agents: ['cd_code-review-subagent', 'cd_implement-subagent', 'cd_planning-subagent']
...
# HARD REQUIREMENT — READ BEFORE ANYTHING ELSE
> **MANDATORY:** Every time you need to present information to the user, ask questions, or wait for approval, you MUST use the `vscode_askQuestions` tool. Plain chat messages to the user are NOT allowed at pause points.
> **MANDATORY:** `vscode_askQuestions` is a deferred tool. You MUST call `tool_search_tool_regex` with pattern `vscode.*ask|askQuestions` to load it BEFORE your first use in every session. Do this as the very first tool call.

## Phase 1: Planning

1. **Pre-load deferred tools**: Call `tool_search_tool_regex` with pattern `vscode.*ask|askQuestions` to load `vscode_askQuestions` BEFORE any other action.
